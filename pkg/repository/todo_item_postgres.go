...
func (r *TodoItemPostgres) Create(listId int, item model.TodoItem) (int, error) {
	...
	createListItemQuery := fmt.Sprintf("INSERT INTO %s (list_id, items_id) VALUES ($1, $2)", listItemsTable)
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]model.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti INNER JOIN %s li on li.items_id = ti.id 
...
func (r *TodoItemPostgres) Update(userid, itemId int, input model.UpdateItemInput) error {
	...
	query := fmt.Sprintf(`UPDATE %s ti SET %s FROM %s li, %s ul
							 WHERE ti.id = li.items_id AND li.list_id = ul.list_id AND ul.user_id = $%d AND ti.id = $%d`,
